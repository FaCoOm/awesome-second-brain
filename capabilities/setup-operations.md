...
| taOSmd | Integration | Install from PyPI or source with pip, download a local embedding model and a local LLM, then run the CLI, the HTTP server via taosmd serve, or the MCP server. | Useful operation depends on the local models and configuration you run. |
